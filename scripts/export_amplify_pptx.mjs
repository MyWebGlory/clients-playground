...
function addPageOne() {
  ...
  // Requested: remove “Executive presence and”, add a bit more space under “AMPLIFY”.
  text(slide, 'High-impact\npublic speaking for life sciences\nleaders', 0.52, 1.70, 3.42, 0.64, { fontFace: 'Cormorant Garamond', fontSize: 14.5, bold: true, color: C.white });
  ...
  // Requested: five items are important → larger font.
  rect(slide, 0.5, 5.05, 3.58, 0.86, C.paper);
  rect(slide, 0.5, 5.05, 0.04, 0.86, C.gold);
  text(slide, 'RxVP panels\nRxVP fireside chats\nStrategic stakeholder engagements\nIndustry conferences\nRxVP/ERG collaborations', 0.66, 5.14, 3.28, 0.74, { fontSize: 9.2, bold: true, color: C.ink });
  ...
  // Move bios to page 2 (page 1 was too dense). Use this area for Program Objectives.
  rect(slide, 0.5, 6.68, 7.5, 3.60, C.paper, { color: 'D7C07A', pt: 0.75 });
...
function addPageTwo() {
  ...
  // Requested: remove "Signature Development Areas" entirely.
  // Use the reclaimed space to move the coach bios here and increase minimum readable font size.
  rect(slide, 0.56, 2.55, 7.38, 5.02, C.paper, { color: 'D7C07A', pt: 0.75 });
  label(slide, 'Our Practitioner Coaches', 0.72, 2.68, 3.6, 0.18, 9.8);

  text(slide, 'Deepa Desai', 0.72, 2.98, 3.35, 0.25, { fontFace: 'Cormorant Garamond', fontSize: 14.5, bold: true, color: C.purple });
  line(slide, 0.72, 3.33, 3.28);
  text(slide, "Dr. Deepa Desai is a globally recognized healthcare executive, public speaker, leadership strategist, and executive presence advisor with more than 20 years of experience leading transformation across the life sciences sector.\n\nShe serves as Founder and CEO of D Cube Consultancy, Head of Asia Pacific for RxVP, Global Board Director at the Healthcare Businesswomen's Association, and has held numerous VP roles in major corporations.\n\nThrough these leadership roles, she helps emerging leaders strengthen executive presence, elevate strategic communication, and speak with confidence, credibility, and influence.\n\nDeepa is a frequent speaker at life sciences conferences and leadership forums across the United States and Europe. She has also expanded the Healthcare Businesswomen’s Association in India to a growing community of 700 members by building panels, summits, and chapters across the country.", 0.72, 3.43, 3.35, 3.96, { fontSize: 8.6, bold: true, color: C.soft });

  text(slide, 'Bonnie Lappin', 4.18, 2.98, 3.35, 0.25, { fontFace: 'Cormorant Garamond', fontSize: 14.5, bold: true, color: C.purple });
  line(slide, 4.18, 3.33, 3.28);
  text(slide, "Bonnie Lappin co-founded the Ambassador Program for the Healthcare Businesswomen's Association in 2013.\n\nDesigned as a global leadership development program for HBA corporate partners, it grew under her leadership from 2013 to 2025 into a signature, often life-changing experience for more than 11,000 participants.\n\nOver those 12 years, Bonnie spoke frequently at life sciences companies across Austria, Belgium, Canada, France, Germany, India, Ireland, Italy, Japan, Malaysia, Singapore, Spain, Switzerland, and the United Kingdom.\n\nShe also led panel development, launch and graduation events, and coached hundreds of panelists and moderators.\n\nToday, Bonnie continues this work through RxVP, a global speakers bureau that curates panels for experienced and emerging speakers.", 4.18, 3.43, 3.35, 3.96, { fontSize: 8.6, bold: true, color: C.soft });

  // Delivery skills (with requested bullet) + customization band at the bottom.
  line(slide, 0.56, 7.72, 7.38);
  label(slide, 'Delivery Skills', 0.56, 7.90, 3.25, 0.18, 9.8);
  bulletList(slide, ['Speaker delivery','Concise storytelling','Audience engagement','Mastering Q&A'], 0.56, 8.18, 3.5, 0.98, { fontSize: 9.2 });
  bulletList(slide, ['Appropriate self-effacing humor','Engagement with panelists, moderators, and interviewers','Enhanced executive presence'], 4.25, 8.18, 3.69, 0.98, { fontSize: 9.2 });

  rect(slide, 0.56, 9.25, 7.38, 0.92, C.lavender);
  rect(slide, 0.56, 9.25, 0.04, 0.92, C.gold);
  label(slide, 'Program design can be customized based on:', 0.8, 9.36, 6.5, 0.18, 9.8);
  bulletList(slide, ['Cohort size','Audience composition and geography','Strategic panel themes'], 0.8, 9.62, 3.5, 0.55, { fontSize: 9.0 });
  bulletList(slide, ['Number and cadence of speaking opportunities','Defined success metrics and developmental outcomes'], 4.25, 9.62, 3.6, 0.55, { fontSize: 9.0 });
  footer(slide);
}
